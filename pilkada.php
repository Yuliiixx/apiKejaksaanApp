...
function tambahLaporan($id_user, $laporan_text, $laporan_pdf, $status, $ktp) {
    ...
    $outputfile = "pdf/" . generateRandomFileName(str_replace(' ', '', 'pilkada',) . '_', '.pdf');
    ...
    $sql = "INSERT INTO tb_pilkada (id_user, laporan_text, laporan_pdf, status,  ktp) VALUES ('$id_user', '$laporan_text', '$outputfile', '$status', '$ktpFile')";
...
function laporanUser($id) {
    global $conn;
    $sql = "SELECT * FROM tb_pilkada WHERE id_user=$id";
    $result = $conn->query($sql);
    $laporan = array();
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
...
function ubahLaporan($id, $laporan_text, $laporan_pdf, $status,  $ktp) {
    ...
    $sql = "UPDATE tb_pilkada SET laporan_text='$laporan_text', status='$status'";
...
function hapusLaporan($id) {
    ...
    $sql = "SELECT laporan_pdf, ktp FROM tb_pilkada WHERE id_laporan = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $sql_delete = "DELETE FROM tb_pilkada WHERE id_laporan = ?";
